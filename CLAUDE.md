...
这是 AI Workspace Dashboard 的公开技能商店。技能文件托管在**腾讯云 COS**（`https://awd-1253816841.cos.ap-beijing.myqcloud.com/skills-registry/`），Dashboard 从 COS 下载技能文件。本 Git 仓库作为技能源文件的版本管理，上架新技能后需通过 COS 上传脚本同步到 COS。
...
Dashboard 从 COS 读取技能文件，**必须执行此步骤**才能在商店中生效：

```bash
node tools/ai-workspace-dashboard/scripts/upload-registry-to-cos.js --type skills --dir tools/skills-registry
```

> 前提：`tools/ai-workspace-dashboard/.env` 中已配置 `COS_SECRET_ID` 和 `COS_SECRET_KEY`。
...
> Git 推送仅用于版本管理和备份，Dashboard 不从 GitHub 读取技能文件。
...
| 只推送 Git 没上传 COS | 商店里看不到新技能 | 必须执行 COS 上传脚本 |
